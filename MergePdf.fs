module MergePdf
    ...
    button_MergePdf.Click.Add(fun _ ->
        if (textbox_InputPdf1.Text = "") then
            MessageBox.Show("Please select input source 1.", "Error - Unspecified source", MessageBoxButtons.OK) |> ignore
        else if (textbox_InputPdf2.Text = "") then
            MessageBox.Show("Please select input source 2.", "Error - Unspecified source", MessageBoxButtons.OK) |> ignore
        else if (PdfReader.TestPdfFile(textbox_InputPdf1.Text) = 0) then
            MessageBox.Show("Source 1 is invalid pdf file.", "Error - Invalid source", MessageBoxButtons.OK) |> ignore
        else if (PdfReader.TestPdfFile(textbox_InputPdf2.Text) = 0) then
            MessageBox.Show("Source 2 is invalid pdf file.", "Error - Invalid source", MessageBoxButtons.OK) |> ignore
        else
            ...
            | DialogResult.OK ->
                ...
                MessageBox.Show("Successfully merged files.", "Success - Merge", MessageBoxButtons.OK) |> ignore
